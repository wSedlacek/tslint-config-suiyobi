...
export class Rule extends AbstractConfigDependentRule {
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new ParameterPropertyWalker(sourceFile, this.ruleName, {
...
class ParameterPropertyWalker extends Lint.AbstractWalker<IOptions> {
  public walk(sourceFile: ts.SourceFile) {
    const cb = (node: ts.Node): void => {
      if (node.kind === ts.SyntaxKind.Constructor) this._checkConstructorDeclaration(<ts.ConstructorDeclaration>node);
  ...
  private _checkConstructorDeclaration(node: ts.ConstructorDeclaration) {
    ...
    if (this.options.allOrNone) {
      ...
      for (let i = index; i < length; ++i) {
        if (utils.isParameterProperty(parameters[i])) {
          if (regular) this.addFailureAtNode(parameters[i], LEADING_FAIL);
        } else {
          regular = true;
        }
      }
      ...
      for (let i = index; i < length; ++i)
        if (!utils.isParameterProperty(parameters[i])) this.addFailureAtNode(parameters[i], TRAILING_FAIL);
    }
