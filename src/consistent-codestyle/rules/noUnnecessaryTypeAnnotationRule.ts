...
function walk(ctx: Lint.WalkContext<IOptions>, checker: ts.TypeChecker) {
  return ts.forEachChild(ctx.sourceFile, function cb(node): void {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.MethodDeclaration:
        if (node.parent!.kind === ts.SyntaxKind.ObjectLiteralExpression) checkObjectLiteralMethod(<ts.MethodDeclaration>node);
        break;
  ...
  function checkFunction(node: FunctionExpressionLike) {
    ...
    if (iife !== undefined) return checkIife(node, iife);
  ...
  function checkContextSensitiveFunctionOrMethod(node: ts.FunctionLikeDeclaration, contextualType: ts.Type) {
    ...
    if (
      ctx.options.checkReturnType &&
      checkReturn &&
      node.type !== undefined &&
      !signatureHasGenericOrTypePredicateReturn(signature) &&
    ...
    for (let i = 0; i < parameters.length; ++i) {
      if (!restParameterContext) {
        ...
        if (context === undefined || context.valueDeclaration === undefined) break;
        if (isTypeParameter(checker.getTypeAtLocation(context.valueDeclaration))) continue;
  ...
  function checkIife(func: FunctionExpressionLike, iife: ts.CallExpression) {
    ...
    outer: for (let i = 0; i < len; ++i) {
      ...
      if (parameter.dotDotDotToken !== undefined) {
        const indexType = declaredType.getNumberIndexType();
        if (indexType === undefined || !typesAreEqual(indexType, contextualType)) break;
        for (let j = i + 1; j < args.length; ++j)
          if (!typesAreEqual(contextualType, checker.getBaseTypeOfLiteralType(checker.getTypeAtLocation(args[j])))) break outer; // TODO build UnionType
        fail(parameter.type);
      } else if (
        compareParameterTypes(contextualType, declaredType, parameter.questionToken !== undefined || parameter.initializer !== undefined)
      ) {
        fail(parameter.type);
      }
    }
  ...
  function checkVariables(list: ts.VariableDeclarationList) {
    ...
    for (const variable of list.declarations) {
      if (variable.type === undefined || variable.initializer === undefined) continue;
      let inferred = checker.getTypeAtLocation(variable.initializer);
      if (!isConst) inferred = checker.getBaseTypeOfLiteralType(inferred);
  ...
  function getContextualTypeOfObjectLiteralMethod(method: ts.MethodDeclaration): ts.Type | undefined {
    ...
    if (!isTypeFlagSet(type, ts.TypeFlags.StructuredType)) return;
  ...
  function signatureHasGenericOrTypePredicateReturn(signature: ts.Signature): boolean {
    if (signature.declaration === undefined) return false;
    if (signature.declaration.type !== undefined && isTypePredicateNode(signature.declaration.type)) return true;
  ...
  function compareParameterTypes(context: ts.Type, declared: ts.Type, optional: boolean): boolean {
    if (optional) declared = removeOptionalityFromType(declared);
    return (
      typesAreEqual(declared, context) || (optional && typesAreEqual(checker.getNullableType(declared, ts.TypeFlags.Undefined), context))
    );
  }

  function isNumericPropertyName(name: ts.PropertyName) {
    const str = getPropertyName(name);
    if (str !== undefined) return isValidNumericLiteral(str) && String(+str) === str;
  ...
  function isAssignableToNumber(type: ts.Type) {
    ...
    return (function check(t): boolean {
      ...
      if (isUnionType(t)) return t.types.every(check);
      if (isIntersectionType(t)) return t.types.some(check);
  ...
  function getMatchingSignature(type: ts.Type, parameters: ReadonlyArray<ts.ParameterDeclaration>): [ts.Signature, boolean] | undefined {
    ...
    const signatures = getSignaturesOfType(type).filter(
      (s) =>
        s.declaration !== undefined && getNumParameters(<ReadonlyArray<ts.ParameterDeclaration>>s.declaration.parameters) >= minArguments
    );
    ...
    switch (signatures.length) {
      ...
      default: {
        ...
        for (let i = 1; i < signatures.length; ++i) {
          ...
          if (str !== sig) {
            if (withoutReturn !== removeSignatureReturn(sig)) return;
            returnUsable = false;
          }
        }
...
function getSignaturesOfType(type: ts.Type): ReadonlyArray<ts.Signature> {
  if (isUnionType(type)) {
    const signatures = [];
    for (const t of type.types) signatures.push(...getSignaturesOfType(t));
    return signatures;
  }
...
function getNumParameters(parameters: ReadonlyArray<ts.ParameterDeclaration>): number {
  ...
  if (parameters[parameters.length - 1].dotDotDotToken !== undefined) return Infinity;
...
function getMinArguments(parameters: ReadonlyArray<ts.ParameterDeclaration>): number {
  ...
  for (; minArguments > 0; --minArguments) {
    ...
    if (parameter.questionToken === undefined && parameter.initializer === undefined && parameter.dotDotDotToken === undefined) break;
  }
  return minArguments;
}
