...
function walk(ctx: Lint.WalkContext<IOptions>) {
  ...
  function isUnused(node: ts.Identifier): boolean {
    if (variables === undefined) variables = collectVariableUsage(ctx.sourceFile);
...
function declaresVariable(
  ...
  if (lastDeclaration.kind === ts.SyntaxKind.Identifier) return lastDeclaration.text === name && isUnused(lastDeclaration);
...
function isSimpleDestructuringForName(pattern: ts.BindingPattern, name: string, isUnused: (node: ts.Identifier) => boolean): boolean {
  ...
  function handleBindingElement(element: ts.BindingElement): boolean | undefined {
    ...
    if (element.name.text !== name) return void identifiersSeen.add(element.name.text);
    if (dependsOnVar !== 0) return false;
    if (element.dotDotDotToken) {
      if (element.parent!.elements.length > 1 || inArray > (element.parent!.kind === ts.SyntaxKind.ArrayBindingPattern ? 1 : 0))
        return false;
    } else if (inArray !== 0) {
      return false;
    }
    if (element.initializer !== undefined && !isUndefined(element.initializer)) return false;
    return !dependsOnPrevious(element) && isUnused(element.name);
  }
  function dependsOnPrevious(element: ts.BindingElement): boolean {
    if (element.propertyName === undefined || element.propertyName.kind !== ts.SyntaxKind.ComputedPropertyName) return false;
    if (isIdentifier(element.propertyName.expression)) return identifiersSeen.has(element.propertyName.expression.text);
    if (isLiteralExpression(element.propertyName.expression)) return false;
