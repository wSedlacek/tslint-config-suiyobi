...
export class Rule extends Lint.Rules.AbstractRule {
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new UnusedWalker(sourceFile, this.ruleName, {
...
class UnusedWalker extends Lint.AbstractWalker<IOptions> {
  public walk(sourceFile: ts.SourceFile) {
    ...
    usage.forEach((variable, identifier) => {
      if (isExcluded(variable, sourceFile, usage, this.options)) return;
      switch (identifier.parent!.kind) {
        ...
        case ts.SyntaxKind.ClassExpression:
          if (variable.uses.length === 0 && this.options.classExpressionName) this._failNamedExpression(identifier, ExpressionKind.Class);
          return;
      }
      if (variable.uses.length === 0) {
        if (
          identifier.text === 'React' &&
          this.sourceFile.languageVariant === ts.LanguageVariant.JSX &&
          isImportFromExternal(identifier) &&
          containsJsx(this.sourceFile)
        )
      ...
      if (uses.length === 0) return this._fail(identifier, 'only written and never read');
      const filtered = uses.length !== variable.uses.length;
      uses = filterUsesInDeclaration(uses, variable.declarations);
      if (uses.length === 0) return this._fail(identifier, `only ${filtered ? 'written or ' : ''}used inside of its declaration`);
...
function filterUsesInDeclaration(uses: VariableUse[], declarations: ts.Identifier[]): VariableUse[] {
  ...
  outer: for (const use of uses) {
    for (const declaration of declarations) {
      ...
      if (
        use.location.pos > parent.pos &&
        use.location.pos < parent.end &&
        (parent.kind !== ts.SyntaxKind.VariableDeclaration || initializerHasNoSideEffect(<ts.VariableDeclaration>parent, use.location))
      )
        continue outer;
    }
    result.push(use);
...
function initializerHasNoSideEffect(declaration: ts.VariableDeclaration, use: ts.Identifier): boolean {
  ...
  return (
    (function cb(node: ts.Node): Result | undefined {
      if (node.pos > use.pos) return Result.NoSideEffect;
      if (node.end <= use.pos) return;
      switch (node.kind) {
...
function filterWriteOnly(uses: VariableUse[], identifier: ts.Identifier): VariableUse[] {
  ...
  for (const use of uses)
    if (
      use.domain & (UsageDomain.Type | UsageDomain.TypeQuery) ||
    ...
    switch (parent.kind) {
      ...
      case ts.SyntaxKind.BinaryExpression:
        if (isAssignmentKind((<ts.BinaryExpression>parent).operatorToken.kind))
          return (
            (<ts.BinaryExpression>parent).right === use &&
...
function isExcluded(variable: VariableInfo, sourceFile: ts.SourceFile, usage: Map<ts.Identifier, VariableInfo>, opts: IOptions): boolean {
  if (variable.exported || variable.inGlobalScope) return true;
  for (const declaration of variable.declarations) {
    ...
    if (declaration.text.startsWith('_')) {
      switch (parent.kind) {
        ...
        case ts.SyntaxKind.VariableDeclaration:
          if (parent.parent!.parent!.kind === ts.SyntaxKind.ForInStatement || parent.parent!.parent!.kind === ts.SyntaxKind.ForOfStatement)
            return true;
          break;
        case ts.SyntaxKind.BindingElement:
          if ((<ts.BindingElement>parent).dotDotDotToken !== undefined) break;
    ...
    if (
      (isParameterDeclaration(parent) && (opts.ignoreParameters || isParameterProperty(parent) || !isFunctionWithBody(parent.parent!))) ||
...
function isImportFromExternal(node: ts.Identifier) {
  switch (node.parent!.kind) {
    case ts.SyntaxKind.ImportEqualsDeclaration:
      if ((<ts.ImportEqualsDeclaration>node.parent).moduleReference.kind === ts.SyntaxKind.ExternalModuleReference) return true;
      break;
...
function typeParameterMayBeRequired(parameter: ts.TypeParameterDeclaration, usage: Map<ts.Identifier, VariableInfo>): boolean {
  ...
  switch (parent.kind) {
    ...
    case ts.SyntaxKind.ClassDeclaration:
      if (typeParameterIsUsed(parameter, usage)) return true;
      if ((<ts.NamedDeclaration>parent).name === undefined) return false;
      const variable = usage.get(<ts.Identifier>(<ts.NamedDeclaration>parent).name)!;
      if (!variable.exported) return variable.inGlobalScope;
  }
  ...
  while (true) {
    switch (parent.kind) {
      ...
      case ts.SyntaxKind.ModuleDeclaration:
        if ((<ts.ModuleDeclaration>parent).name.kind !== ts.SyntaxKind.Identifier) return ts.isExternalModule(parent.getSourceFile());
        if (parent.flags & ts.NodeFlags.GlobalAugmentation) return true;
...
function typeParameterIsUsed(parameter: ts.TypeParameterDeclaration, usage: Map<ts.Identifier, VariableInfo>): boolean {
  if (usage.get(parameter.name)!.uses.length !== 0) return true;
  const parent = <ts.ClassDeclaration | ts.InterfaceDeclaration>parameter.parent;
  if (parent.name === undefined) return false;
  ...
  for (const declaration of usage.get(parent.name)!.declarations) {
    ...
    if (declarationParent === parent) continue;
    switch (declarationParent.kind) {
      case ts.SyntaxKind.ClassDeclaration:
      case ts.SyntaxKind.InterfaceDeclaration:
        if (
          declarationParent.typeParameters !== undefined &&
