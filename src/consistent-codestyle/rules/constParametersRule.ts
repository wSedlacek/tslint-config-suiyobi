...
function walk(ctx: Lint.WalkContext<void>): void {
  collectVariableUsage(ctx.sourceFile).forEach((variable, identifier) => {
    if (!isParameter(identifier.parent!) || !isConst(identifier, ctx.sourceFile)) return;
...
function isConst(name: ts.Identifier, sourceFile: ts.SourceFile) {
  if (name.parent!.kind === ts.SyntaxKind.Parameter) return getJsDoc(name.parent!, sourceFile).some(jsDocContainsConst);
  // destructuring
...
function jsDocContainsConst(jsDoc: ts.JSDoc): boolean {
  if (jsDoc.tags !== undefined)
    for (const tag of jsDoc.tags) if (tag.tagName.text === 'const' || tag.tagName.text === 'constant') return true;
  return false;
}
