...
function walk(ctx: Lint.WalkContext<IOptions>) {
  ...
  return ts.forEachChild(sourceFile, function cb(node): void {
    if (isIfStatement(node) && (!ignoreConstructor || !isConstructorClosestFunctionScopeBoundary(node))) check(node);
...
function diff(start: number, end: number, sourceFile: ts.SourceFile): number {
  return ts.getLineAndCharacterOfPosition(sourceFile, end).line - ts.getLineAndCharacterOfPosition(sourceFile, start).line + 1;
}
...
function getCaseClauseExit(
  ...
  return statements[statements.length - 1].kind === ts.SyntaxKind.BreakStatement
    ...
    clause.parent!.clauses[clause.parent!.clauses.length - 1] === clause && isLastStatement(node, statements)
    ? 'break'
    : undefined;
}
...
function isConstructorClosestFunctionScopeBoundary(node: ts.Node): boolean {
  ...
  while (currentParent) {
    if (isFunctionScopeBoundary(currentParent)) return currentParent.kind === ts.SyntaxKind.Constructor;
