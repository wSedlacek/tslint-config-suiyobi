...
import { AbstractReturnStatementWalker } from '../walker';
...
export class Rule extends Lint.Rules.AbstractRule {
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new ReturnWalker(sourceFile, this.ruleName, {
...
class ReturnWalker extends AbstractReturnStatementWalker<IOptions> {
  protected _checkReturnStatement(node: ts.ReturnStatement) {
    if (node.expression !== undefined && this._isUndefined(node.expression)) this.addFailureAtNode(node.expression, FAIL_MESSAGE);
  }
...
function isUndefinedNotVoidExpr(expression: ts.Expression): boolean {
  if (utils.isIdentifier(expression) && expression.text === 'undefined') return true;
