...
function walk(ctx: Lint.WalkContext<void>) {
  ...
  variables.forEach((variable, identifier) => {
    if (identifier.parent!.kind !== ts.SyntaxKind.EnumDeclaration || seen.has(identifier)) return;
    const track: IEnum = {
  ...
  for (const { track, declaration } of declarations) {
    for (const member of declaration.members) {
      const isConst = track.isConst || member.initializer === undefined || isConstInitializer(member.initializer, track.members, findEnum);
      ...
      if (!isConst) track.canBeConst = false;
    }
  }
  for (const track of enums) {
    if (track.isConst || !track.canBeConst || !onlyConstUses(track)) continue;
  ...
  function findEnum(name: ts.Identifier): IEnum | undefined {
    for (const track of enums) {
      if (track.name !== name.text) continue;
      for (const use of track.uses) if (use.location === name) return track;
    }
    return;
  }
}
...
function onlyConstUses(track: IEnum): boolean {
  for (const use of track.uses) {
    if (use.domain & UsageDomain.Type || use.domain === UsageDomain.Namespace) continue;
    if (use.domain & UsageDomain.TypeQuery) return false;
    ...
    switch (parent.kind) {
      ...
      case ts.SyntaxKind.ElementAccessExpression:
        ...
        if (
          (<ts.ElementAccessExpression>parent).argumentExpression === undefined || // compatibilty with typescript@<2.9.0
...
function isStringValued(initializer: ts.Expression, members: Map<string, IEnumMember>, findEnum: FindEnum): boolean {
  return (function stringValued(node): boolean {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.PropertyAccessExpression:
        return findEnum(<ts.Identifier>(<ts.PropertyAccessExpression>node).expression)!.members.get(
          (<ts.PropertyAccessExpression>node).name.text
        )!.stringValued;
      case ts.SyntaxKind.ElementAccessExpression:
        return findEnum(<ts.Identifier>(<ts.ElementAccessExpression>node).expression)!.members.get(
          (<ts.Identifier>(<ts.ElementAccessExpression>node).argumentExpression).text
        )!.stringValued;
      default:
        // StringLiteral
        return true;
