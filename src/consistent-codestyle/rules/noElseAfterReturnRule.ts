...
export class Rule extends Lint.Rules.AbstractRule {
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new IfWalker(sourceFile, this.ruleName, {
...
function shouldCheckNode(node: ts.IfStatement, allowElseIf: boolean): node is ts.IfStatement & { elseStatement: {} } {
  ...
  if (!allowElseIf) return !isElseIf(node);
  if (isIfStatement(node.elseStatement) && isElseIf(node.elseStatement)) return false;
  while (isElseIf(node)) {
    node = node.parent;
    if (!endsWithReturnStatement(node.thenStatement)) return false;
  }
  return true;
}
