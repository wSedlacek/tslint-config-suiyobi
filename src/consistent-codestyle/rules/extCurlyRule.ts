...
export class Rule extends Lint.Rules.AbstractRule {
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new ExtCurlyWalker(sourceFile, this.ruleName, {
...
class ExtCurlyWalker extends Lint.AbstractWalker<IOptions> {
  ...
  private _checkLoop(node: ts.IterationStatement) {
    if (this._needsBraces(node.statement)) {
      if (node.statement.kind !== ts.SyntaxKind.Block) this.addFailureAtNode(node.statement, FAIL_MESSAGE_MISSING);
  ...
  private _checkIfStatement(node: ts.IfStatement) {
    ...
    if (then) {
      if (node.thenStatement.kind !== ts.SyntaxKind.Block) this.addFailureAtNode(node.thenStatement, FAIL_MESSAGE_MISSING);
    ...
    if (otherwise) {
      if (
        ...
        node.elseStatement.kind !== ts.SyntaxKind.Block &&
  ...
  private _needsBraces(node: ts.Statement, allowIfElse?: boolean): boolean {
    if (isBlock(node)) return node.statements.length !== 1 || this._needsBraces(node.statements[0], allowIfElse);
    if (!allowIfElse && this.options.nestedIfElse && isIfStatement(node) && node.elseStatement !== undefined) return true;
    if (!this.options.child) return false;
    if (isIfStatement(node)) {
      const result = this._ifStatementNeedsBraces(node);
      return result[0] || result[1];
    }
    if (isIterationStatement(node) || isLabeledStatement(node)) return this._needsBraces(node.statement);
  ...
  private _ifStatementNeedsBraces(node: ts.IfStatement, excludeElse?: boolean): [boolean, boolean] {
    if (this.options.else) {
      if (node.elseStatement !== undefined || isElseIf(node)) return [true, true];
    } else if (this.options.consistent) {
      if (
        this._needsBraces(node.thenStatement) ||
        (!excludeElse &&
          node.elseStatement !== undefined &&
        ...
        return [true, true];
      if (isElseIf(node) && this._ifStatementNeedsBraces(node.parent, true)[0]) return [true, true];
    }
...
function unwrapBlock(node: ts.Statement): ts.Statement {
  while (isBlock(node) && node.statements.length === 1) node = node.statements[0];
  return node;
}
