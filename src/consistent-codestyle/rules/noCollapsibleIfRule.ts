...
class CollapsibleIfWalker extends AbstractIfStatementWalker<void> {
  protected _checkIfStatement(node: ts.IfStatement) {
    if (node.elseStatement === undefined) {
      ...
      if (utils.isBlockLike(then) && then.statements.length === 1) then = then.statements[0];
      ...
      utils.isBlock(node.elseStatement) &&
      node.elseStatement.statements.length === 1 &&
      utils.isIfStatement(node.elseStatement.statements[0])
    ) {
      this.addFailure(node.elseStatement.pos - 4, (<ts.IfStatement>node.elseStatement.statements[0]).thenStatement.pos, FAIL_MERGE_ELSE_IF);
    }
  }
}
