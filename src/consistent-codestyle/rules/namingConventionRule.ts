...
import { AbstractConfigDependentRule } from '../rules';
...
class NormalizedConfig {
  ...
  constructor(raw: RuleConfig) {
    ...
    this._specifity = (Specifity[raw.type as any] as unknown) as number;
    this._modifiers = 0;
  ...
  public matches(type: TypeSelector, modifiers: number, name: string): boolean {
    if (this._final && type > this._type << 1)
      // check if TypeSelector has a higher bit set than this._type
      return false;
    if ((this._type & type) === 0 || (this._modifiers & ~modifiers) !== 0) return false;
...
class NameChecker {
  ...
  public check(name: ts.Identifier, walker: Lint.AbstractWalker<any>) {
    ...
    if (this._regex !== undefined && !this._regex.test(identifier)) walker.addFailureAtNode(name, this._failMessage(REGEX_FAIL));

    if (this._leadingUnderscore) {
      if (identifier[0] === '_') {
        if (this._leadingUnderscore === 'forbid') walker.addFailureAtNode(name, this._failMessage(LEADING_FAIL));
    ...
    if (this._trailingUnderscore) {
      if (identifier[identifier.length - 1] === '_') {
        if (this._trailingUnderscore === 'forbid') walker.addFailureAtNode(name, this._failMessage(TRAILING_FAIL));
  ...
  private _checkPrefixes(identifier: string, name: ts.Identifier, prefixes: string[], walker: Lint.AbstractWalker<any>): string {
    for (const prefix of prefixes) if (identifier.startsWith(prefix)) return identifier.slice(prefix.length);
  ...
  private _checkSuffixes(identifier: string, name: ts.Identifier, suffixes: string[], walker: Lint.AbstractWalker<any>): string {
    for (const suffix of suffixes) if (identifier.endsWith(suffix)) return identifier.slice(0, -suffix.length);
...
class IdentifierNameWalker extends Lint.AbstractWalker<NormalizedConfig[]> {
  ...
  private _isUnused(name: ts.Identifier): boolean {
    if (this._usage === undefined) this._usage = utils.collectVariableUsage(this.sourceFile);
  ...
  private _checkTypeParameters(
    ...
    if (node.typeParameters !== undefined)
      for (const { name } of node.typeParameters) this._checkName(name, TypeSelector.genericTypeParameter, modifiers);
  }
  ...
  public visitEnumDeclaration(node: ts.EnumDeclaration) {
    ...
    for (const { name } of node.members) if (utils.isIdentifier(name)) this._checkName(name, TypeSelector.enumMember, modifiers);
  }
  ...
  public visitClassLikeDeclaration(node: ts.ClassLikeDeclaration) {
    if (node.name !== undefined) this._checkDeclaration(<ts.ClassLikeDeclaration & { name: ts.Identifier }>node, TypeSelector.class);
    this._checkTypeParameters(node, Modifiers.global);
  }

  public visitMethodDeclaration(node: ts.MethodDeclaration) {
    if (isNameIdentifier(node)) this._checkDeclaration(node, TypeSelector.method);
  ...
  public visitParameterDeclaration(node: ts.ParameterDeclaration) {
    if (node.parent!.kind === ts.SyntaxKind.IndexSignature) return;
    if (isNameIdentifier(node)) {
      ...
      utils.forEachDestructuringIdentifier(<ts.BindingPattern>node.name, (declaration) => {
        ...
        if (!isEqualName(declaration.name, declaration.propertyName)) modifiers |= Modifiers.rename;
        if (utils.isFunctionWithBody(node.parent!) && this._isUnused(declaration.name)) modifiers |= Modifiers.unused;
  ...
  public visitPropertyDeclaration(node: ts.PropertyDeclaration) {
    if (isNameIdentifier(node)) this._checkDeclaration(node, TypeSelector.property);
  }

  public visitSetAccessor(node: ts.SetAccessorDeclaration) {
    if (isNameIdentifier(node)) this._checkDeclaration(node, TypeSelector.accessor);
  }
  ...
  private _checkVariableDeclarationList(list: ts.VariableDeclarationList, modifiers: number) {
    ...
    if ((list.flags & ts.NodeFlags.Const) !== 0) modifiers |= Modifiers.const;
    utils.forEachDeclaredVariable(list, (declaration) => {
      ...
      if (this._isUnused(declaration.name)) currentModifiers |= Modifiers.unused;
      if (isFunctionVariable(declaration)) selector = TypeSelector.functionVariable;
  ...
  private _createChecker(type: TypeSelector, modifiers: number, name: string): NameChecker | null {
    const config = this.options.reduce(
      (format: IFormat, rule) => {
        if (!rule.matches(type, modifiers, name)) return format;
    ...
    if (!config.leadingUnderscore && !config.trailingUnderscore && !config.format && !config.prefix && !config.regex && !config.suffix)
  ...
  private _getModifiers(node: ts.Node, type: TypeSelector, modifiers: Modifiers = 0): number {
    if (node.modifiers !== undefined) {
      if (type & Types.member) {
        // property, method, parameter property
        ...
        if (utils.hasModifier(node.modifiers, ts.SyntaxKind.ReadonlyKeyword)) modifiers |= Modifiers.const;
        if (utils.hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword)) modifiers |= Modifiers.static;
      }
      if (utils.hasModifier(node.modifiers, ts.SyntaxKind.ConstKeyword))
        // stuff like const enums
        modifiers |= Modifiers.const;
      if (utils.hasModifier(node.modifiers, ts.SyntaxKind.ExportKeyword)) modifiers |= Modifiers.export;
      if (utils.hasModifier(node.modifiers, ts.SyntaxKind.AbstractKeyword)) modifiers |= Modifiers.abstract;
    }

    if (type !== TypeSelector.property && type !== TypeSelector.method) modifiers |= this._depth !== 0 ? Modifiers.local : Modifiers.global;
...
function matchesAnyFormat(identifier: string, formats: Format[]): boolean {
  for (const format of formats) if (matchesFormat(identifier, format)) return true;
  return false;
}
...
function formatFormatList(formats: Format[]): string {
  ...
  for (let i = 1; i < lastIndex; ++i) result += ', ' + formats[i];
...
function isEqualName(name: ts.Identifier, propertyName?: ts.PropertyName) {
  return propertyName === undefined || (propertyName.kind === ts.SyntaxKind.Identifier && propertyName.text === name.text);
}
