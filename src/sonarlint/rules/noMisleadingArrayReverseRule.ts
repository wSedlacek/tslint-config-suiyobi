...
class Visitor extends TypedSonarRuleVisitor {
  ...
  private isArrayMutatingCall(expression: ts.PropertyAccessExpression): boolean {
    return isArray(expression.expression, this.program.getTypeChecker()) && ARRAY_MUTATING_CALLS.includes(expression.name.text);
  }
  ...
  private isForbiddenOperation(node: ts.Node): boolean {
    const { parent } = node;
    return parent != null && parent.kind !== ts.SyntaxKind.ExpressionStatement && !firstLocalAncestor(node, ts.SyntaxKind.ReturnStatement);
  }
