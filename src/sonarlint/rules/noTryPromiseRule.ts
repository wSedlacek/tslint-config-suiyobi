...
const isThened = (callExpr: ts.Expression) => isPropertyAccessExpression(callExpr.parent) && callExpr.parent.name.getText() === 'then';

const isCaught = (callExpr: ts.Expression) => isPropertyAccessExpression(callExpr.parent) && callExpr.parent.name.getText() === 'catch';

const isCatch = (callExpr: ts.Expression) =>
  isCallExpression(callExpr) && isPropertyAccessExpression(callExpr.expression) && callExpr.expression.name.getText() === 'catch';
