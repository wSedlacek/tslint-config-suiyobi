...
export class Rule extends tslint.Rules.TypedRule {
  ...
  public static formatMessage(expression: Omit<ts.Expression, '_expressionBrand'>) {
...
class Visitor extends TypedSonarRuleVisitor {
  ...
  private isReturnValueUsed(node: ts.CallExpression) {
    ...
    if (isBinaryExpression(parent)) {
      if (parent.operatorToken.kind === ts.SyntaxKind.AmpersandAmpersandToken || parent.operatorToken.kind === ts.SyntaxKind.BarBarToken) {
  ...
  private endOfPropertyChain(expression: ts.Expression): Omit<ts.Expression, '_expressionBrand'> {
