...
import { isIdentifier, isFunctionLikeDeclaration, isBlock, isExpressionStatement, isCallExpression } from '../utils/nodes';
...
function getPromiseExecutor(node: ts.NewExpression) {
  if (isIdentifier(node.expression) && node.expression.text === 'Promise' && node.arguments && node.arguments.length === 1) {
