...
class Visitor extends TypedSonarRuleVisitor {
  ...
  raiseIssue(
    ...
    if (declaration && (declaration.parameters as ts.TextRange).pos && declaration.getSourceFile() === node.getSourceFile()) {
      const textRange: ts.TextRange = declaration.parameters as ts.TextRange;
      issue.addSecondaryLocation(new IssueLocation(textRange.pos, textRange.end, node.getSourceFile(), 'Formal parameters'));
    }
  }
