...
class Visitor extends SonarRuleVisitor {
  visitParameterDeclaration(paramDeclaration: ts.ParameterDeclaration) {
    if (!paramDeclaration.initializer && isOptionalBoolean(paramDeclaration) && !isInterfaceMethodParameter(paramDeclaration)) {
...
function usesUnionUndefinedOptionalSyntax(type?: ts.TypeNode) {
  ...
  const { types } = type;
  return (
    types.length === 2 && types.some((t) => is(t, ts.SyntaxKind.BooleanKeyword)) && types.some((t) => is(t, ts.SyntaxKind.UndefinedKeyword))
  );
}
