...
class Visitor extends SonarRuleVisitor {
      ...
      if (!this.isRawExclusion(text)) {
        ...
        if (parsed.parseDiagnostics.length === 0 && parsed.statements.length > 0 && !this.isExclusion(parsed)) {
          this.addIssueAtLocation(
            new IssueLocation(group[0].pos, group[group.length - 1].end, this.sourceFile, `Remove this commented out code.`)
          );
        }
      }
    });
  ...
  private isExpressionExclusion(node: ts.Node) {
    return (
      ...
      (!node.getText().endsWith(';') ||
        (nodes.isBinaryExpression(node.expression) && nodes.is(node.expression.operatorToken, ts.SyntaxKind.CommaToken)) ||
  ...
  private isReturnThrowExclusion(node: ts.Node) {
    return (nodes.isReturnStatement(node) || nodes.isThrowStatement(node)) && !!node.expression && nodes.isIdentifier(node.expression);
  }
