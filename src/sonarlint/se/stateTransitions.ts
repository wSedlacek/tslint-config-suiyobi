...
import { numericLiteralSymbolicValue, simpleSymbolicValue, undefinedSymbolicValue, objectLiteralSymbolicValue } from './symbolicValues';
...
export function applyExecutors(
  ...
  function binaryExpression(expression: ts.BinaryExpression) {
    return nodes.isAssignmentKind(expression.operatorToken.kind) ? assignmentLike(expression) : state.pushSV(simpleSymbolicValue());
  }

  function assignmentLike(expression: ts.BinaryExpression) {
    return expression.operatorToken.kind === ts.SyntaxKind.EqualsToken ? assignment(expression) : compoundAssignment(expression);
  }
