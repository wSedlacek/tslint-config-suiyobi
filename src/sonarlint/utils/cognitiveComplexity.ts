...
class ComplexityWalker extends TreeVisitor {
  ...
  private visitLoop(nestedNode: ts.Node, complexityKeyword: ts.SyntaxKind, ...notNestedExpressions: (ts.Node | undefined)[]) {
  ...
  private static isAndOrOr(node: ts.Node): boolean {
    if (is(node, ts.SyntaxKind.BinaryExpression)) {
      return is((node as ts.BinaryExpression).operatorToken, ts.SyntaxKind.AmpersandAmpersandToken, ts.SyntaxKind.BarBarToken);
    }
    return false;
  }
  ...
  private flattenLogicalExpression(node: ts.Node): ts.BinaryExpression[] {
    if (ComplexityWalker.isAndOrOr(node)) {
      ...
      return this.flattenLogicalExpression(leftChild).concat([binaryExpression]).concat(this.flattenLogicalExpression(rightChild));
    }
