 ...
 */
export default function areEquivalent(first: ts.Node | ts.Node[], second: ts.Node | ts.Node[], ignoreLiterals = false): boolean {
  if (isNode(first) && isNode(second)) {
    ...
    return first.length === second.length && first.every((leftNode, index) => areEquivalent(leftNode, second[index], ignoreLiterals));
  } else {
    return false;
  }
}
