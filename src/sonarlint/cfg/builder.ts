...
class CfgBuilder {
  ...
  private createWhileRootBlock(whileLoop: ts.WhileStatement, firstLoopStatementBlock: CfgBlock, current: CfgBlock): CfgBlock {
  ...
  private buildExpression(current: CfgBlock, expression: ts.Expression): CfgBlock {
    switch (expression.kind) {
      ...
      case SyntaxKind.TemplateExpression:
        ...
        [...templateExpression.templateSpans].reverse().forEach((span) => (current = this.buildExpression(current, span.expression)));
        return current;
      ...
      case SyntaxKind.PostfixUnaryExpression:
        current.addElement(expression);
        return this.buildExpression(current, (expression as ts.PrefixUnaryExpression | ts.PostfixUnaryExpression).operand);
  ...
  private forLoopLabel(forLoop: ts.ForStatement) {
    return 'for(' + textOrEmpty(forLoop.initializer) + ';' + textOrEmpty(forLoop.condition) + ';' + textOrEmpty(forLoop.incrementor) + ')';
