...
class RuleWalker extends Lint.RuleWalker {
  ...
  protected visitArrowFunction(node: ts.ArrowFunction): void {
    ...
    if (arrowBody.kind === ts.SyntaxKind.Block) {
      ...
      if (
        ...
        returnExpression &&
        this.isObjectLiteral(returnExpression)
      ) {
        return;
      }
      ...
      if (this.always || (this.asNeeded && this.requireReturnForObjectLiteral && this.isObjectLiteral(arrowBody))) {
  ...
  private report(arrowBody: ts.Node, expected: boolean) {
    ...
    const failure = this.createFailure(arrowBody.getStart(), arrowBody.getWidth(), `${val} block statement surrounding arrow body.`);
