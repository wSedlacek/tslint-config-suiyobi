...
function isKind<T extends ts.Node>(node: ts.Node, kind: string): node is T {
  return node.kind === (ts.SyntaxKind[kind as any] as any);
}

function isOneOf(node: ts.Node, kinds: string[]) {
  return kinds.some((kind) => node.kind === (ts.SyntaxKind[kind as any] as any));
}
...
class IndentWalker extends Lint.RuleWalker {
    ...
    if (isKind<ts.IfStatement>(node, 'IfStatement')) {
      ...
      if (elseStatement) {
        const elseKeyword = node
          .getChildren()
          .filter((ch) => isKind(ch, 'ElseKeyword'))
          .shift()!;
      ...
      if (handler) {
        const catchKeyword = handler
          .getChildren()
          .filter((ch) => isKind(ch, 'CatchKeyword'))
          .shift()!;
      ...
      if (finalizer) {
        const finallyKeyword = node
          .getChildren()
          .filter((ch) => isKind(ch, 'FinallyKeyword'))
          .shift()!;
        this.checkNodeIndent(finallyKeyword, neededIndent);
      }
    } else if (isKind(node, 'DoStatement')) {
      const whileKeyword = node
        .getChildren()
        .filter((ch) => isKind(ch, 'WhileKeyword'))
        .shift()!;
  ...
  private blockIndentationCheck(node: ts.BlockLike | ts.IterationStatement): void {
    ...
    const functionLike = ['FunctionExpression', 'FunctionDeclaration', 'MethodDeclaration', 'Constructor', 'ArrowFunction'];
    ...
    if (isKind<ts.IfStatement>(node, 'IfStatement') && !isKind<ts.Block>(node.thenStatement, 'Block')) {
      ...
      } else if (node.parent && isOneOf(node.parent, ['ClassDeclaration', 'ClassExpression', 'InterfaceDeclaration', 'TypeLiteral'])) {
  ...
  private isNodeBodyBlock(node: ts.Node): node is ts.BlockLike {
    const hasBlock = ['ClassDeclaration', 'ClassExpression', 'InterfaceDeclaration', 'TypeLiteral'];
    return isKind<ts.Block>(node, 'Block') || (isKind<ts.SyntaxList>(node, 'SyntaxList') && isOneOf(node.parent!, hasBlock));
  }
  ...
  private isOuterIIFE(node: ts.FunctionExpression): boolean {
    ...
    return (
      (isKind<ts.ExpressionStatement>(stmt, 'ExpressionStatement') || isKind<ts.VariableStatement>(stmt, 'VariableStatement')) &&
      !!stmt.parent &&
      isKind(stmt.parent, 'SourceFile')
    );
  }
  ...
  private isArgBeforeCalleeNodeMultiline(node: ts.Node): boolean {
    ...
    if (parent.kind === ts.SyntaxKind.CallExpression) {
      ...
      if (callExpression && callExpression['arguments'].length >= 2 && callExpression['arguments'][1] === node) {
   ...
   */
  private getParentNodeByType<T extends ts.Node>(node: ts.Node, kind: number, stopAtList: number[] = [ts.SyntaxKind.SourceFile]): T | null {
    let parent = node.parent;

    while (parent && parent.kind !== kind && stopAtList.indexOf(parent.kind) === -1 && parent.kind !== ts.SyntaxKind.SourceFile) {
  ...
  protected checkIndentInArrayOrObjectBlock(node: ts.Node): void {
    ...
    let elements: ts.Node[] = isKind(node, 'ObjectLiteralExpression') ? (node as any)['properties'] : node['elements'];
    ...
    if (this.isNodeFirstInLine(node) && node.parent) {
      ...
      if (parentVarNode && this.getLine(parentVarNode) !== nodeLine) {
        ...
        if (
          !isKind(parent, 'VariableDeclaration') ||
          parentVarNode === (parentVarNode.parent as ts.VariableDeclarationList).declarations[0]
        ) {
  ...
  protected isNodeInVarOnTop(node: ts.Node, varNode: ts.VariableDeclaration) {
    ...
    return varNode && parentLine === nodeLine && (varNode.parent! as ts.VariableDeclarationList).declarations.length > 1;
  }
  ...
  protected visitVariableStatement(node: ts.VariableStatement) {
    ...
    if (tokenBeforeLastElement && isKind(tokenBeforeLastElement, 'CommaToken')) {
      ...
      const declaratorIndent =
        typeof OPTIONS.VariableDeclarator[varKind] === 'number' ? OPTIONS.VariableDeclarator[varKind] : DEFAULT_VARIABLE_INDENT;
  ...
  private checkFunctionMethodExpression(node: ts.MethodDeclaration | ts.ConstructorDeclaration | ts.FunctionExpression) {
