  ...
  public walk(sourceFile: ts.SourceFile) {
    const cb = (node: ts.Node): void => {
      if (node.kind === ts.SyntaxKind.ElementAccessExpression) {
        this.checkNode(node, node.getChildAt(1), node.getChildAt(3));
      } else if (node.kind === ts.SyntaxKind.ComputedPropertyName) {
        this.checkNode(node, node.getChildAt(0), node.getChildAt(2));
      }
    ...
    if (this.options.always) {
      ...
      if (beforeWhitespaceLength !== 0) {
        this.addFailureAtNode(
          leftBracketNode,
          NEVER_AFTER_MESSAGE,
          Lint.Replacement.deleteText(leftBracketNode.getEnd(), beforeWhitespaceLength)
        );
      }
      ...
      if (afterWhitespaceLength !== 0) {
        this.addFailureAtNode(
          rightBracketNode,
          NEVER_BEFORE_MESSAGE,
          Lint.Replacement.deleteText(rightBracketNode.getStart() - afterWhitespaceLength, afterWhitespaceLength)
        );
      }
    }
  }
