  ...
  public walk(sourceFile: ts.SourceFile) {
    const cb = (node: ts.Node): void => {
      ...
      if (ts.isBinaryExpression(node)) {
        ...
        if (this.isComparisonOperator(nodeExpr) && this.hasSameToken(nodeExpr.left, nodeExpr.right)) {
          this.addFailureAt(nodeExpr.operatorToken.getStart(), nodeExpr.operatorToken.getWidth(), Rule.FAILURE_STRING);
        }
      } else {
