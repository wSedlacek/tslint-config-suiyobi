...
class NoMultiSpacesWalker extends Lint.RuleWalker {
  ...
  private STRING_TYPES = [ts.SyntaxKind.NoSubstitutionTemplateLiteral, ts.SyntaxKind.LastTemplateToken, ts.SyntaxKind.StringLiteral];
  private exceptions: { [key: string]: boolean } = {};
  private src: string;
  private targets: number[] = [];
  private lastNode: ts.Node;
  private targetNode: { [key: number]: ts.SourceFile | ts.Node } = {};
  ...
  private warn(value: string, pos: number, node: ts.Node): void {
    ...
    const exceptionName = this.EXCEPTION_MAP[
      node.parent!.kind as ts.SyntaxKind.VariableDeclaration | ts.SyntaxKind.PropertyAssignment | ts.SyntaxKind.BinaryExpression
    ];
