...
class ValidJsdocWalker extends Lint.RuleWalker {
  ...
  private startFunction(node: ts.Node) {
    ...
    if (node.kind === ts.SyntaxKind.ArrowFunction && (node as ts.ArrowFunction).body.kind !== ts.SyntaxKind.Block) returnPresent = true;

    if (this.isTypeClass(node)) returnPresent = true;
  ...
  private addReturn(node: ts.ReturnStatement) {
    ...
    while (parent && nodes.indexOf(parent) === -1) parent = parent.parent;
  ...
  private checkJSDoc(node: ts.Node) {
    ...
    if (!comments || start === undefined || width === undefined) return;
    ...
    if (
      ...
      !hasConstructor &&
      node.parent &&
      node.parent.kind !== ts.SyntaxKind.GetKeyword &&
      !this.isTypeClass(node)
    ) {
