...
class RuleWalker extends Lint.RuleWalker {
  ...
  private exitFunctionExpression(node: ts.FunctionExpression) {
    ...
    if (
      ...
      const failure = this.createFailure(node.getStart(), node.getWidth(), 'Unexpected function expression.');
  ...
  protected visitNode(node: ts.Node) {
    ...
    if (info && node.parent && node.parent.kind !== ts.SyntaxKind.FunctionExpression) {
      if (node.kind === ts.SyntaxKind.ThisKeyword) {
        ...
        (node.kind === ts.SyntaxKind.PropertyAccessExpression || node.kind === ts.SyntaxKind.MetaProperty) &&
