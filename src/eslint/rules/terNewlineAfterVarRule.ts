    ...
    const onNode = (node: ts.Node): void => {
      ...
      if (lastVariableStatementNode) {
        ...
        if (isNewLineRequired && expectedLineFixes[0]) {
          ...
          this.addFailureAt(lastVariableStatementNode.getStart(), 1, EXPECTED_BLANK_LINE_MESSAGE, expectedLineFixes);
        } else if (unexpectedLineFixes[0]) {
          this.addFailureAt(lastVariableStatementNode.getStart(), 1, UNEXPECTED_BLANK_LINE_MESSAGE, unexpectedLineFixes);
        }
