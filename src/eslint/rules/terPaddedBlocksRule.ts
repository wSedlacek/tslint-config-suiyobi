...
class RuleWalker extends Lint.AbstractWalker<ITerPaddedBlocksOptions> {
  ...
  private checkPadding(node: ts.Node): void {
    ...
    if (paddingAllowed ? !openPadded : openPadded) {
      const openFix = Lint.Replacement.replaceFromTo(openBraceReplacement.from, openBraceReplacement.to, paddingAllowed ? '\n\n' : '\n');
      this.addFailure(
    ...
    if (paddingAllowed ? !closePadded : closePadded) {
      const closeFix = Lint.Replacement.replaceFromTo(closeBraceReplacement.from, closeBraceReplacement.to, paddingAllowed ? '\n\n' : '\n');
      this.addFailure(
