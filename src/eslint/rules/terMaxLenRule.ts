...
function isTrailingComment(line: string, lineNumber: number, comment: INode) {
  return (
    comment &&
    comment.start[0] === lineNumber &&
    lineNumber <= comment.end[0] &&
...
class MaxLenWalker extends Lint.RuleWalker {
  ...
  public findFailures(sourceFile: ts.SourceFile) {
    ...
    const ignoreTrailingComments = this.getOption(IGNORE_TRAILING_COMMENTS) || this.getOption(IGNORE_COMMENTS) || false;
