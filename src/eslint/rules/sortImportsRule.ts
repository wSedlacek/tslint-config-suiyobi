...
class RuleWalker extends Lint.RuleWalker {
  ...
  private static _processMemberSyntaxSortOrder(sortOption: string[]): MemberSyntaxType[] {
    const defaultOrder = [
      MemberSyntaxType.None,
      MemberSyntaxType.All,
      MemberSyntaxType.Multiple,
      MemberSyntaxType.Single,
      MemberSyntaxType.Alias,
    ];
    if (Array.isArray(sortOption) && typeof sortOption[0] === 'string' && sortOption.length === 5) {
      const memberSyntaxTypeMap: { [key: string]: MemberSyntaxType } = {
      ...
      const usedOptions: { [key: string]: string } = {};
