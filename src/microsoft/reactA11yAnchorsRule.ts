...
function walk(ctx: Lint.WalkContext<Options>) {
  ...
  function validateAllAnchors(): void {
    ...
    while (anchorInfoList.length > 0) {
      ...
      anchorInfoList.forEach((anchorInfo: IAnchorInfo): void => {
        if (
          current.href &&
        ...
        if (
          ...
          ctx.addFailureAt(anchorInfo.start, anchorInfo.width, DIFFERENT_HREF_DIFFERENT_TEXT_FAILURE_STRING + firstPosition(current));
        }
      });
    }
  }
