...
const aria: { [attributeName: string]: Partial<IAria> } = ariaSchema;
...
function walk(ctx: Lint.WalkContext<void>) {
  function cb(node: ts.Node): void {
    if (tsutils.isJsxAttribute(node)) {
      ...
      const allowUndefined: boolean = (aria[propName].allowUndefined !== undefined ? aria[propName].allowUndefined : false) ?? false;
