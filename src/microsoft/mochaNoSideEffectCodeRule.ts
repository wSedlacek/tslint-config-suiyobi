...
function walk(ctx: Lint.WalkContext<Options>) {
  ...
  function validateExpression(initializer: ts.Expression, parentNode: ts.Node): void {
    ...
    if (initializer.kind === ts.SyntaxKind.CallExpression && AstUtils.getFunctionTarget(<ts.CallExpression>initializer) === 'moment()') {
      return;
    }
    // new Date is OK
  ...
  if (MochaUtils.isMochaTest(ctx.sourceFile)) {
    ctx.sourceFile.statements.forEach((statement: ts.Statement): void => {
