...
export default function (options: Schema): Rule {
  return (host: Tree, context: SchematicContext) => {
    return chain([extendTSLintRules(options.path || './tslint.json', options.rules)])(host, context);
  };
}
...
function extendTSLintRules(filePath: string, selectedRules: string[]) {
  return (tree: Tree) => {
    ...
    const extendFrom = recommended ? 'ngrx-tslint-rules/recommended' : 'ngrx-tslint-rules';
    ...
    if (!recommended) {
      ...
      const configContent = fs.readFileSync(path.join(__dirname, 'rules-config.json'), 'utf-8');
      ...
      const rulesToAdd = selectedRules
        ...
        .reduce((rules, name) => {
          ...
          return {
            ...rules,
            [name]: {
              severity: `${ruleInfo.type === 'functionality' ? 'error' : 'warning'}`,
            },
          };
        }, {});
