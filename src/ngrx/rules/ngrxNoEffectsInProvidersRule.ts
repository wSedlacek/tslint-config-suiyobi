...
export class Rule extends Lint.Rules.TypedRule {
  public static metadata: Lint.IRuleMetadata = {
    ...
    descriptionDetails: 'If an Effect is registered to the EffectsModule and is added as a provider, it will be registered twice',
    options: null,
  ...
  public static FAILURE_STRING = 'The Effect should not be listed as a provider if it is added to the EffectsModule';
  ...
  public applyWithProgram(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    const effectIdentifierNodes = tsquery(sourceFile, Rule.QUERY) as ts.Identifier[];

    const effectNames = effectIdentifierNodes.map((node) => node.text).join('|');
    const hits = tsquery(sourceFile, `${NG_MODULE_PROVIDERS} > Identifier[name=/${effectNames}/]`);
