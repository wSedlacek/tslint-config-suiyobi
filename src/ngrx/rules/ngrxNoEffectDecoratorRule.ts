...
export class Rule extends Lint.Rules.TypedRule {
  ...
  public static FAILURE_STRING = 'The createEffect creator function is preferred';
  ...
  public applyWithProgram(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    ...
    const failures = hits.map(
      (node): Lint.RuleFailure =>
        new Lint.RuleFailure(sourceFile, node.getStart() - 1, node.getStart() + node.getWidth(), Rule.FAILURE_STRING, this.ruleName)
    );
