...
export class Rule extends Lint.Rules.TypedRule {
  ...
  public applyWithProgram(sourceFile: ts.SourceFile, program: ts.Program): Lint.RuleFailure[] {
    const arrayAsReturnStatementHits = tsquery(sourceFile, `${this.EFFECTS_OPERATORS_QUERY} ReturnStatement > ArrayLiteralExpression`);
    const arrayShorthandStatementHits = tsquery(sourceFile, `${this.EFFECTS_OPERATORS_QUERY} > ArrowFunction > ArrayLiteralExpression`);
    ...
    const failures = hits
      .map((node): Lint.RuleFailure | null => {
        const { typeArguments } = (typeChecker.getTypeAtLocation(node) as unknown) as ts.NodeWithTypeArguments;

        const returnsMultipleActions =
          typeArguments &&
          typeArguments.some((args: NodeWithTypes) => args.types && args.types.some((type) => couldBeType(type, 'Action')));

        if (returnsMultipleActions) {
          return new Lint.RuleFailure(sourceFile, node.getStart(), node.getStart() + node.getWidth(), Rule.FAILURE_STRING, this.ruleName);
        }
