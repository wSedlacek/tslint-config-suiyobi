...
class Test {
  ...
  private compareErrors(expectedErrors: LintFailure[], foundErrors: LintFailure[], linter: Lint.Linter): void {
    ...
    if (this.testFixer && this.output) {
      const fixes = linter
        .getResult()
        .failures.filter((f) => f.hasFix())
        .map((f) => f.getFix()!);
      const fixedCode = Lint.Replacement.applyFixes(this.code, fixes);
      const fixerMsg = [`Fixer output mismatch in ${this.name}:`, '', codeStr, '', '      '].join('\n');
...
class RuleTester {
   ...
   * @return {RuleTester}
   */
  public addTestGroupWithConfig(name: string, description: string, groupConfig: any, tests: (ITest | string)[]): this {
...
export { dedent, Position, Failure, TestGroup, RuleTester, readFixture };
