...
it(`adds ngrx-tslint-rules to array extends`, () => {
  const tslint = setup(`{"extends": ["tslint:latest", "ngrx-tslint-rules/recommended"]}`, ['recommended']);
...
it(`the recommended config has priority over rules`, () => {
  const tslint = setup(`{}`, ['recommended', 'ngrx-action-hygiene', 'ngrx-no-duplicate-action-types']);
...
it(`adds the given rules to an empty config`, () => {
  const tslint = setup(`{}`, ['ngrx-action-hygiene', 'ngrx-no-duplicate-action-types']);
...
it(`adds the given rules to the existing config`, () => {
  const tslint = setup(`{"rules": {"foo": true, "bar": false}}`, ['ngrx-action-hygiene', 'ngrx-no-duplicate-action-types']);
...
function setup(content: string, rules: string[]) {
  ...
  const schematicRunner = new SchematicTestRunner('ngrx-tslint-rules', collectionPath);
