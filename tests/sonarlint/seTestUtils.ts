...
function executeFromSource(source: string) {
  const { sourceFile, program } = parseString(source);
  const result = execute(build(Array.from(sourceFile.statements))!, SymbolTableBuilder.build(sourceFile, program), ProgramState.empty());
  if (!result) {
...
function findInspectCall(result: ExecutionResult) {
  for (const [programPoint, programStates] of result.programNodes.entries()) {
    if (isCallExpression(programPoint) && isIdentifier(programPoint.expression) && programPoint.expression.text === '_inspect') {
