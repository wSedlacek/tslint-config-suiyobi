...
function recursion(condition: boolean): number {
  ...
  else return recursion(condition);
}

function nesting_func_no_complexity(condition: boolean) {
  // Ok
...
function nesting_class_no_complexity(condition: boolean) {
  // Ok
  class MyInnerClass {
    private nested_func() {
      //          ^^^^^^^^^^^ {{Refactor this method to reduce its Cognitive Complexity from 1 to the 0 allowed.}} [[cost:1]]
...
function nesting_func_with_function_expression(condition: boolean) {
  ...
  let lambda_expression = (x: boolean) => {
    // (nesting level +1)
    if (x) {
    } // +2
  };
