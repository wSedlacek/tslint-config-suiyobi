...
class Exceptions {
  ...
  public setW(w: string) {
    // Compliant, w does not exist
    this.x = w;
  }

  public getY(someParam: number) {
    // Compliant, not a zero-parameters getter
    return 3;
  }

  public setZ(y: string, someParam: number) {
    // Compliant, not a one-parameters setter
    this.z = 3;
  }
  ...
  public getZ() {
    // Compliant, does not match "return this.?;" pattern
    this.setZ('', 1);
  }

  public set v(z: number) {
    // Compliant, does not match "this.? =" pattern
    this.x;
  }

  public get v() {
    // Compliant, not a single return statement
    if (this.z) {
      return 1;
    } else {
      return this.z;
