 ...
 */
export function runRuleWithLintFile(Rule: any, testDirectoryName: string, testFileName: string, ...ruleArguments: any[]) {
...
function checkRule(Rule: any, lintFileName: string, ruleArguments: any[]) {
  ...
  const { errors: actualErrors, secondaryLocations: actualSecondaryLocations } = runRuleOnFile(Rule, lintFileName, ruleArguments);
...
function runRuleOnFile(Rule: any, file: string, ruleArguments: any[]) {
  ...
  if ((rule as tslint.Rules.TypedRule).applyWithProgram) {
    ...
    failures = rule.apply(parseString(source, path.extname(file) === '.ts' ? ScriptKind.TS : ScriptKind.TSX).sourceFile);
  }
